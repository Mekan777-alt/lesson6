...
def gen_users_hobbies(users_path, hobbies_path, sum_path):
    ...
    for i in range(users_list_length):
        user = next(user_gen)
    ...
    if hobbies_list_length > users_list_length:
        with open(sum_path, 'a', encoding='utf-8') as saved_file:
            saved_file.write('code 1')
        print('Exit code 1: Неожиданный конец списка пользователей')
    else:
...
if len(argv) == 2:
    if argv[1] == 'gen':
        ...
        print('gen - сгенерировать списки хобби в файл по умолчанию\n'
              ...
              'если список хобби больше списка пользователей, всё сломается!')
elif len(argv) == 3:
    if argv[1] == 'gen':
        gen_hobbies(argv[2])
        ...
        print('Exit code 2: Указаны неверные аргумент')
    if exists(argv[1]) and exists(argv[2]):
        gen_users_hobbies(argv[1], argv[2], argv[3])
    else:
        print('Exit code 3: Вы указали не существующий файл')
